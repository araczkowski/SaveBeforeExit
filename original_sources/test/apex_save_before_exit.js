(function($) {
  $.widget('ui.apex_save_before_exit', {
    ...
    _create: function() {
      ...
      // TODO change console.debug to apex.debug
      console.debug('revertModificationsSelector: ' + uiw.options.revertModificationsSelector);
      ...
      if (uiw.options.debug) {
        ...
        if ($revertElmts.length) {
          $revertElmts.each(function() {
            console.debug('...Ignore modifications to: "' + this.nodeName + '"');

            for (var name in this.attributes) {
              if (typeof(this.attributes[name]) !== 'function') {
                console.debug('......' + this.attributes[name].nodeName + ': "' + this.attributes[name].nodeValue + '"');
              }
            }
          });
        } else {
          console.debug('...No elements selected to ignore modifications to');
        }
      }
      //
      ...
      // on change
      $(document).on('change', uiw.options.revertModificationsSelector, function(event) {
        // check if we have modification on page
        if (apex.jQuery(document).apex_save_before_exit('modificationDetected')) {

          var r = confirm(uiw.options.saveMessage);
          if (r == true) {
